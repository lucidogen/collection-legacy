...
import { input, set, signal, state, unset, when } from 'cerebral/operators'
...
( collection: CollectionPreType
  ...
  const { draftPath, dynamicPaths, errorPath } = collection.paths
    ...
    , { success:
        ...
        , { true:
            [ firebase.put
              ...
              , input`imageFile`
              , { progress: signal`${collection.name}.uploadProgress`
                }
              )
            , { success:
